...
Definisco le quantità di interesse.
...
Anziché implementare le equazioni 23.3 e 23.4 in $\mathsf{R}$, possiamo usare la funzione `summarize_normal_normal()` del pacchetto `bayesrules`. 

Gli argomenti richiesti da `summarize_normal_normal()` sono stati specificati sopra. Con tali informazioni possiamo replicare i due risultati precedenti.
...
Avendo trovato che $p(\mu \mid \sigma) = \mathcal{N}(\mu = 14.86107, \sigma = 0.2883797)$, diventa possibile rispondere alla domanda del problema: "Si trovi il valore massimo della densità della distribuzione a posteriori $p(\mu \mid y)$".

Il problema chiede di trovare il valore massimo dell'ordinata della funzione di densità $\mathcal{N}(\mu = 14.86107, \sigma = 0.2883797)$. Il massimo della funzione di densità Normale si trova in corrispondenza della media della distribuzione. Dunque
...
Svolgo ora lo stesso esercizio usando Stan. Nel caso presente è inutile usare Stan, dato che la soluzione può essere trovata più facilmente usando le formule 23.3 e 23.4. Tuttavia, svolgo comunque l'esercizio usando Stan per verificare che viene trovata la stessa soluzione.
