...
Si notino i messaggi che vengono forniti alla conclusione del campionamento MCMC. In particolare, `All 4 chains finished successfully.` ci dice che il campionamento è andato a buon fine, nel senso che non ci sono stati problemi di convergenza. Se non otteniamo questo messaggio questo vuol dire che non c'è stata convergenza, ovvero che i valori che sono stati generati *non* costituiscono un campione casuale della distribuzione a posteriori target. Di conseguenza, se non otteniamo il messaggio precedente, non è possibile interpretare i risulati ottenuti.

## Esaminare i risultati

L'oggetto `fit` contiene molteplici informazioni. Per manipolarlo più facilmente lo trasformiamo nel formato `stanfit`. Ciò ci consentirà di usare varie "utility functions" per estrarre dall'oggetto `stanfit` le informazioni di nostro interesse. La funzione che possiamo usare a questo fine è `rstan::read_stan_csv()`.

```{r}
fit_stanfit <- rstan::read_stan_csv(fit$output_files())
```

L'oggetto `fit_stanfit` è una lista che contiene 4 elementi

```{r}
length(fit_stanfit@sim$samples)
```

I quattro elementi corrispondono alle 4 catene di 4000 osservazioni ciascuna, le quali contengono valori casuali estratti dalla distribuzione a posteriori di $\theta$. Per esempio, i valori iniziali della prima catena di Markov sono i seguenti.

```{r}
head(fit_stanfit@sim$samples[[1]])
```

Estraggo le stime di $\theta$.

```{r}
chain_1 <- fit_stanfit@sim$samples[[1]]$theta
```

Creo una rappresentazione grafica della catena nei primi 100 passi.

```{r}
plot(1:100, chain_1[1:100], type = 'l')
```

L'oggetto `chain_1` è composto da 4000 elementi.

```{r}
length(chain_1)
```

Un istogramma degli ultimi 2000 elementi (escludendo il warm-up) si ottiene nel modo seguente.

```{r}
hist(chain_1[2001:4000])
```

Lo stesso si può dire delle altre 3 catene che sono state create.

Se combiniamo i valori delle 4 catene, escludendo il warm-up, possiamo creare un grafico che riporta la stima della distribuzione a posteriori nel modo seguente:
...
Di default, gli estremi dell'intervallo che contiene la curva corrispondono al 100% dell'area. L'area centrale evidenziata corrisponde al 50% dell'area. 
Il segmento verticale evidenziato corrisponde alla mediana.

Se vogliamo evidenziare il 90% dell'area centrale sottesa alla curva, usiamo l'argomento `prob = 0.9`.

```{r}
mcmc_areas(posterior, pars = c("theta"), prob = 0.9)
```
Il valore numerico esatto di questo intervallo si ottiene nel modo seguente.

```{r}
fit$summary(c("theta"))
```

L'esame del "mixing" delle catene è fornito dal seguente grafico.

```{r}
fit_stanfit %>%
  mcmc_trace(pars = c("theta"))
```

Un istogramma si ottiene nel modo seguente

```{r}
 mcmc_hist(fit_stanfit, pars = "theta") +
  yaxis_text(TRUE) +
  ylab("count")
```
...
```{r}
mcmc_dens(fit_stanfit, pars = "theta") +
  yaxis_text(TRUE) +
  ylab("density") +
