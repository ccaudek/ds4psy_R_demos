---
title: "Data science per psicologi - demo 19.01"
...
- `RStan` - integrazione con il linguaggio $\textsf{R}$;
...
- `rstanarm`: fornisce un sostituto per i modelli frequentisti forniti da base $\textsf{R}$ e `lme4` utilizzando la sintassi usata in $\textsf{R}$ per la specificazione dei modelli statistici;
- `edstan`: modelli Stan per la Item Response Theory;
- `cmdstanr`, un'interfaccia $\textsf{R}$ per `CmdStan`.
