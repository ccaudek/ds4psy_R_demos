...
Esamineremo qui l'andamento della funzione nell'intervallo di valori $x$ compreso tra 70 e 160:
...
Come nel caso della Binomiale, teniamo costante i dati -- nel caso presente, il valore $x = 114$. Il parametro $\sigma$ è assunto noto, per cui ad esso attribuiamo il valore 15. Rimane il valore (ignoto) del parametro $\mu$. Anche in questo caso, dunque, procediamo come nel caso della verosimiglianza della Binomiale, ovvero ci chiediamo quale forma assuma la funzione di verosimiglianza al variare dei valori del parametro incognito. Nella presente simulazione consideriamo 1000 possibile valori del parametro $\mu$, ovvero i valori compresi nell'intervallo $[70, 160]$. Applichiamo dunque 1000 volte la formula della densità Gaussiana, una volta per ciascuno dei 1000 possibili valori $\mu$, tenendo costanti gli altri valori nella formula, ovvero $x = 114$ e $\sigma = 15$. Otteniamo così 1000 punti (ovvero, 1000 coppie $x$ e $f(x)$). La curva che interpola tali punti è la funzione di verosimiglianza.

```{r}
fx <- dnorm(x, mean = mu, sd = 15)
```
...
Consideriamo ora il caso di 30 osservazioni indipendenti iid, ciascuna delle quali può essere considerata come la realizzazione di una variabile casuale Gaussiana di media comune sconosciuta e di deviazione standard 15.
...
Generiamo i dati assumendo $\mu = 100$:
...
Abbiamo visto in precedenza come, nel caso di due variabi casuali, si può costruire la funzione distribuzione congiunta nel modo seguente: $p(X_1, X_2) = p(X_1) p(X_2 \mid X_1)$. Se le due variabili casuali sono indipendenti, abbiamo semplicemente $p(X_1, X_2) = p(X_1) p(X_2)$. Possiamo generalizzare tale risultato dicendo che, nel caso di $n$ v.c. iid, cascuna distribuita come $\mathcal{N}(\mu, \sigma)$, avremo che
...
Nella funzione così costruita, facciamo nuovamente variare il valore dei parametri (in questo caso, solo il possibile valore $\mu$)  e teniamo costante tutto il resto (ovvero, i dati e $\sigma$). Così facendo otteniamo la funzione di log-verosmiglianza che in $\mathsf{R}$ si può scrivere così:
...
Nel caso presente, consideriamo valori $\mu$ compresi nell'intervallo $[70, 130]$:
...
Per ciascuno di questi possibili valori del parametro $\mu$ calcoliamo il valore della funzione congiunta di verosimiglianza di tutte le 30 osservazioni. Salviamo i risultati nel vettore `ll`:
...
Generiamo un grafico della funzione di log-verosimiglianza:
...
Troviamo ora la stima di massima verosimiglianza. 

```{r}
mu[which.max(ll)]
```

Sappiamo che la stima di massima verosimiglianza coincide con la media del campione:
